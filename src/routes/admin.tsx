...
import { loadOperator, saveOperator } from "@/lib/admin.server";
import { PAYMENT } from "@/lib/site";
...
function AdminDesk() {
  ...
  const [stripeLink, setStripeLink] = useState(PAYMENT.stripePaymentLink);
  ...
  useEffect(() => {
    ...
    void loadOperator().then((data) => {
      setStripeLink(data.stripeLink || PAYMENT.stripePaymentLink);
      ...
      <main className="mx-auto max-w-3xl space-y-8 px-4 py-12 sm:px-6">
        ...
        <section className="rounded-3xl bg-surface p-6 shadow-(--shadow-card)">
          ...
          <ul className="mt-3 space-y-2 text-sm text-muted">
            <li>Checkout is Stripe-hosted — no PAN/CVC fields on OxlisVoid.</li>
            ...
            <li>Set Stripe success URL to your live domain + /checkout?paid=1</li>
            <li>Turn on 3D Secure in the Stripe dashboard.</li>
          </ul>
        </section>
        ...
        <form onSubmit={onSave} className="space-y-4 rounded-3xl bg-surface p-6 shadow-(--shadow-card)">
          ...
          <label className="block text-sm">
            <span className="mb-1.5 block text-muted">Stripe Payment Link</span>
            <input
              value={stripeLink}
